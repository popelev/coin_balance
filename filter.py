...
cutoff = datetime.datetime(2023, 8, 25)
...
def _search_block_with_cutoff_ts(
    ...
    if abs(delta.seconds)>= 14: # Precision within 3 min
...
if __name__ == "__main__":
    ...
    def run():
        ...
        start = time.time()
        start_block = scanner.search_block_with_cutoff_ts(end_block=end_block, cutoff_ts=cutoff_ts, search_step_size=search_step_size, get_block=scanner.web3.eth.get_block)
        duration = time.time() - start
        blocks_to_scan = end_block - start_block

        logger.info(f"Scanning events from block {start_block} ({datetime.datetime.fromtimestamp(scanner.web3.eth.get_block(start_block).timestamp)}) - {end_block} ({datetime.datetime.fromtimestamp(scanner.web3.eth.get_block(end_block).timestamp)}), blocks_to_scan: {blocks_to_scan}. Scan envelop estimation took {duration} seconds")
